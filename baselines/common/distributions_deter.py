...
class CategoricalPdType(PdType):
    ...
    def pdfromlatent(self, latent_vector, init_scale=1.0, init_bias=0.0):
        ...
        return self.pdclass()(pdparam, self.epsilon), pdparam
...
class DiagGaussianPdType(PdType):
    ...
    def pdfromlatent(self, latent_vector, init_scale=1.0, init_bias=0.0):
        ...
        logstd = tf.constant(value=np.log(0.3), name='pi/logstd', shape=[1, self.size], dtype=np.float32)
...
class CategoricalPd(Pd):
    ...
    def neglogp(self, x):
        base=tf.constant(self.epsilon, shape=[self.logits.shape.as_list()[0]])
        good=tf.cast(tf.math.equal(tf.argmax(self.logits, axis=-1, output_type=x.dtype), x), tf.float32) *tf.constant(1.-2*self.epsilon,shape=[self.logits.shape.as_list()[0]])
        return -tf.log(base+good)
    ...
    def sample(self):
        epsi = tf.random_uniform(shape=[tf.shape(self.logits)[0]], dtype=tf.float32) - tf.constant(self.epsilon)
        epsi = tf.cast((tf.sign(epsi)+1)/2, dtype=tf.int64)
        u = tf.random_uniform(shape=[tf.shape(self.logits)[0]], minval=0, maxval=tf.shape(self.logits, out_type=tf.int64)[1], dtype=tf.int64)
        return tf.multiply((1-epsi), u) + tf.multiply(epsi, tf.argmax(self.logits, axis=-1))
...
def make_pdtype(ac_space, epsilon):
    from gym import spaces
    if isinstance(ac_space, spaces.Box):
        assert len(ac_space.shape) == 1
        return DiagGaussianPdType(ac_space.shape[0])
    elif isinstance(ac_space, spaces.Discrete):
        return CategoricalPdType(ac_space.n, epsilon)
